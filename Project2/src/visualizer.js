  ...
  // 1 - populate the audioData array with the frequency data from the analyserNode
    ...
    if(params.showCircles){
        ...
        for(let i =0; i < audioData.length; i++){
            ...
            let circleRadius = percentage * maxRadius;
            ctx.beginPath();
            ctx.fillStyle = utils.makeColor(255,111,.34-percentage/3.0);
            ctx.arc(canvasWidth/2, canvasHeight/2, circleRadius,0,2*Math.PI, false);
            ctx.fill();
            ctx.closePath();

            ctx.beginPath();
            ctx.fillStyle = utils.makeColor(0,0,255,.10-percentage/10);
            ...
            ctx.save();
            ctx.beginPath();
            ctx.fillStyle = utils.makeColor(200,200,0,.5-percentage/5);
